...
class YandexResult(OneResult):
    ...
    def bbox(self):
        ...
        if envelope:
            ...
            try:
                return self._get_bbox(float(south), float(west), float(north), float(east))
            except:
                pass
...
class YandexQuery(MultipleResultsQuery):
    ...
    def _adapt_results(self, json_response):
        return [item["GeoObject"] for item in json_response["response"]["GeoObjectCollection"]["featureMember"]]
