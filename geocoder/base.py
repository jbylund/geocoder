...
class OneResult(object):
    ...
    def confidence(self):
        if self.bbox:
            ...
            for score, maximum in [(10, 0.25), (9, 0.5), (8, 1), (7, 5), (6, 7.5), (5, 10), (4, 15), (3, 20), (2, 25)]:
                if distance < maximum:
                    return score
                if distance >= 25:
                    return 1
...
class MultipleResultsQuery(MutableSequence):
    ...
    def __init__(self, location, **kwargs):
        ...
        if not self._is_valid_result_class():
            raise ValueError("Subclass must define _RESULT_CLASS from 'OneResult'. Got %s", self._RESULT_CLASS)
    ...
    def __repr__(self):
        base_repr = "<[{0}] {1} - {2} {{0}}>".format(self.status, self.provider.title(), self.method.title())
        if len(self) == 0:
    ...
    def _connect(self):
        ...
        try:
            ...
            self.response = response = self.rate_limited_get(
                self.url, params=self.params, headers=self.headers, timeout=self.timeout, proxies=self.proxies
            )
    ...
    def __getattr__(self, name):
        ...
        if self.current_result is None:
            raise AttributeError("%s not found on %s, and current_result is None".format(name, self.__class__.__name__))
